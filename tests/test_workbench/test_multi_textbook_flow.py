...
def test_multi_textbook_topic_fusion_survives_restart_and_stales_shared_topics(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert all(
            Path(item["stored_path"]).parent == course_root / "教材原文件" for item in imported
        )
        ...
        for source in imported:
            ...
            for chapter in chapters:
                assert (
                    _expect_ok(client.post(f"/api/workbench/chapters/{chapter['id']}/confirm"))[
                        "status"
                    ]
                    == "CONFIRMED"
                )
        ...
        confirmed = _expect_ok(client.post(f"/api/workbench/courses/{course['id']}/topics/confirm"))
    ...
    with TestClient(_app_factory(db_path, fs_root)) as restarted:
        ...
        snapshots = {}
        for topic in topics:
            blocks = _expect_ok(restarted.get(f"/api/workbench/topics/{topic['id']}/note-blocks"))
            cards = _expect_ok(restarted.get(f"/api/workbench/topics/{topic['id']}/cards"))
            runs = _expect_ok(restarted.get(f"/api/workbench/topics/{topic['id']}/runs"))
        ...
        assert (
            _expect_ok(restarted.post(f"/api/workbench/chapters/{shared_chapter_id}/confirm"))[
                "status"
            ]
            == "CONFIRMED"
        )
        ...
        stale_topics = _expect_ok(restarted.get(f"/api/workbench/courses/{course['id']}/topics"))
        assert [topic["status"] for topic in stale_topics] == ["STALE", "STALE"]
        for topic in stale_topics:
            assert (
                _expect_ok(restarted.get(f"/api/workbench/topics/{topic['id']}/note-blocks"))
                == snapshots[topic["id"]][0]
            )
            assert (
                _expect_ok(restarted.get(f"/api/workbench/topics/{topic['id']}/cards"))
