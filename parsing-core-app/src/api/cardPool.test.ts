...
describe("card pool API contract", () => {
  ...
  it("validates card metadata and sends CAS edits", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/workbench/cards/c1",
  ...
  it("sends favorite changes with the current version", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue({ ok: true, status: 200, json: async () => ({ ...card, favorite: true, updated_at: 4 }) }),
    );
    const api = await import("./workbench");
    await api.setCourseCardFavorite("c1", true, 3);
    expect(fetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/workbench/cards/c1/favorite",
      expect.objectContaining({ method: "PATCH", body: JSON.stringify({ favorite: true, expected_updated_at: 3 }) }),
    );
  });
});
