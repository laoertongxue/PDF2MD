...
const state = {
  chapters: [
    { id: "ch1", source_id: "s1", course_id: "c1", seq: 0, title: "战略", status: "DRAFT", start: 0, end: 100 },
  ],
  fingerprint: "fp-1",
};
...
it("reads, replaces and confirms chapter drafts using the backend snapshot contract", async () => {
  const fetchMock = vi
    ...
    .mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          chapters: state.chapters.map((chapter) => ({ ...chapter, status: "CONFIRMED" })),
          fingerprint: "fp-3",
        }),
        { status: 200 },
      ),
  ...
  await expect(
    replaceChapterDrafts("s1", "fp-1", [{ id: "ch1", title: "战略管理", start: 0, end: 100 }]),
  ).resolves.toMatchObject({ fingerprint: "fp-2" });
  ...
  expect(fetchMock).toHaveBeenNthCalledWith(
    ...
    expect.objectContaining({
      ...
      body: JSON.stringify({
        ...
        chapters: [{ id: "ch1", title: "战略管理", start: 0, end: 100 }],
      }),
    }),
  );
  expect(fetchMock).toHaveBeenNthCalledWith(
...
it("rejects malformed chapter boundaries from the service", async () => {
  vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
    new Response(JSON.stringify({ chapters: [{ ...state.chapters[0], start: 100, end: 10 }], fingerprint: "fp" }), {
      status: 200,
    }),
  );
