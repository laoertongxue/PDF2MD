...
const loadSources = vi
  .fn()
  .mockResolvedValue([
    { id: "s1", course_id: "c1", kind: "main", file_path: "/a.pdf", title: "战略教材", status: "READY" },
  ]);
vi.mock("../../store/useWorkbenchStore", () => ({
  useWorkbenchStore: () => ({
    ...
    selectedCourseId: "c1",
    sources: {
      c1: [{ id: "s1", course_id: "c1", kind: "main", file_path: "/a.pdf", title: "战略教材", status: "READY" }],
    },
  }),
}));
...
beforeEach(() => {
  ...
  vi.spyOn(api, "replaceChapterDrafts").mockImplementation(async (_sourceId, _fingerprint, chapters) => ({
    fingerprint: "fp-2",
    chapters: chapters.map((chapter, seq) => ({
      ...chapter,
      id: chapter.id ?? `new-${seq}`,
      source_id: "s1",
      course_id: "c1",
      seq,
      status: "DRAFT",
  ...
  vi.spyOn(api, "confirmChapterDrafts").mockImplementation(async () => ({
    ...drafts(),
    fingerprint: "fp-3",
    chapters: drafts().chapters.map((chapter) => ({ ...chapter, status: "CONFIRMED" })),
  }));
});
afterEach(cleanup);
...
it("edits, reorders, splits, merges and saves one source snapshot", async () => {
  ...
  const firstName = requireAt(names, 0, "chapter name input");
  await userEvent.clear(firstName);
  await userEvent.type(firstName, "战略导论");
  await userEvent.click(within(source).getByRole("button", { name: "下移 战略导论" }));
  await userEvent.click(requireAt(within(source).getAllByRole("button", { name: /拆分/ }), 0, "split button"));
  expect(within(source).getAllByRole("textbox", { name: "章节名称" })).toHaveLength(3);
  await userEvent.click(requireAt(within(source).getAllByRole("button", { name: /合并下一章/ }), 0, "merge button"));
  const starts = within(source).getAllByRole("spinbutton", { name: "起始边界" });
  const secondStart = requireAt(starts, 1, "second chapter start input");
  await userEvent.clear(secondStart);
  await userEvent.type(secondStart, "90");
  await userEvent.click(within(source).getByRole("button", { name: "保存章节草稿" }));
  await waitFor(() =>
    expect(api.replaceChapterDrafts).toHaveBeenCalledWith(
      "s1",
      "fp-1",
      expect.arrayContaining([expect.objectContaining({ start: 90 })]),
    ),
  );
});
...
it("confirms the saved fingerprint and locks all editing controls", async () => {
  ...
  expect(
    requireAt(within(source).getAllByRole("textbox", { name: "章节名称" }), 0, "chapter name input"),
  ).toBeDisabled();
