...
export default function CardPool() {
  ...
  useEffect(() => {
    loadCourses().catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "课程加载失败"));
  }, [loadCourses]);
  useEffect(() => {
  ...
  const visibleCards = useMemo(() => {
    ...
    return cards.filter(
      (card) =>
        ...
        (!keyword ||
          [card.title, card.content, card.origin_title, ...card.tags].some((value) =>
  ...
  const replaceCard = (updated: Card) =>
    setCards((current) => current.map((card) => (card.id === updated.id ? updated : card)));
  const toggleFavorite = async (card: Card) => {
    setError(null);
    try {
      replaceCard(await api.setCourseCardFavorite(card.id, !card.favorite, card.updated_at));
    } catch (reason) {
  ...
  const saveEdit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "卡片保存失败");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-5 animate-in">
      ...
      <div className="flex flex-wrap items-center gap-2 border-y border-zinc-200 py-3">
        <label className="relative min-w-60 flex-1">
          ...
          <span className="sr-only">搜索课程卡片</span>
          <input
            type="search"
            aria-label="搜索课程卡片"
            value={query}
            ...
            className="h-9 w-full border border-zinc-300 bg-white pl-9 pr-3 text-sm outline-none focus:border-zinc-500"
          />
        </label>
        <div className="inline-flex border border-zinc-200 bg-zinc-50 p-0.5" role="group" aria-label="卡片来源筛选">
          {(
            ...
            <button
              ...
              className={`h-8 px-3 text-sm ${origin === value ? "bg-white font-medium shadow-sm" : "text-zinc-500"}`}
            >
              {label}
            </button>
          ))}
        ...
        <select
          ...
          className="h-9 border border-zinc-300 bg-white px-3 text-sm"
        >
        ...
        <button
          ...
          className={`inline-flex h-9 items-center gap-1.5 border px-3 text-sm ${favoritesOnly ? "border-amber-300 bg-amber-50 text-amber-800" : "border-zinc-300 bg-white text-zinc-600"}`}
        >
      ...
      {error && (
        <p role="alert" className="border border-red-100 bg-red-50 px-3 py-2 text-sm text-red-600">
          {error}
        </p>
      )}
      {routeMessage && (
        <p
          role="status"
          aria-live="polite"
          className="border-l-2 border-emerald-500 bg-emerald-50 px-3 py-2 text-sm text-emerald-800"
        >
          {routeMessage}
        </p>
      )}
      {cards.length === 0 ? (
        ...
        <div className="border border-dashed border-zinc-300 bg-white px-8 py-12 text-center">
          <p className="text-sm font-medium text-zinc-700">没有符合条件的卡片</p>
          <button
            type="button"
            onClick={() => {
              setQuery("");
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {visibleCards.map((card) => (
            <article
              ...
              data-highlighted={highlightedCardId === card.id ? "true" : undefined}
              className={`border bg-white p-4 outline-none transition-shadow ${highlightedCardId === card.id ? "border-emerald-500 ring-2 ring-emerald-200" : "border-zinc-200"}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    {card.tags.map((item) => (
                      <span key={item} className="bg-zinc-100 px-1.5 py-0.5 text-xs text-zinc-500">
                        {item}
                      </span>
                    ))}
                    {card.status === "ARCHIVED" && (
                      <span className="bg-zinc-200 px-1.5 py-0.5 text-xs text-zinc-600">已归档</span>
                    )}
                  </div>
                </div>
                <div className="flex shrink-0">
                  <button
                    ...
                    className="p-1.5 text-zinc-500 hover:text-amber-600"
                  >
                    <Star className={`h-4 w-4 ${card.favorite ? "fill-amber-400 text-amber-500" : ""}`} />
                  </button>
                  <button
                    type="button"
                    title="编辑卡片"
              ...
              <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-zinc-600">{card.content}</p>
              <div className="mt-3 flex items-center justify-between gap-3">
                <span className="text-xs text-zinc-400">{card.card_type}</span>
                <Link
                  to={
                    card.origin_type === "chapter"
                      ? `/workbench/chapter?chapterId=${card.origin_id}`
      ...
      {editing && (
        ...
        >
          <form onSubmit={saveEdit} className="w-full max-w-xl border border-zinc-200 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-5 py-3">
              <h2 id="card-edit-title" className="text-base font-semibold">
                编辑卡片
              </h2>
            ...
            <div className="space-y-4 p-5">
              <label className="block text-sm">
                ...
                <input
                  ...
                  className="mt-1 h-9 w-full border border-zinc-300 px-3"
                />
              </label>
              <label className="block text-sm">
                正文
                <textarea
                  ...
                  rows={8}
                  className="mt-1 w-full resize-y border border-zinc-300 p-3"
                />
              </label>
              <label className="block text-sm">
                标签
            ...
            <div className="flex justify-end gap-2 border-t px-5 py-3">
              <button
                ...
                className="h-9 border border-zinc-300 px-4 text-sm"
              >
                取消
              </button>
              <button
                type="submit"
                disabled={saving}
                className="h-9 bg-zinc-900 px-4 text-sm text-white disabled:opacity-50"
              >
                {saving ? "保存中" : "保存"}
              </button>
            </div>
...
function EmptyCourse() {
  return (
    <div className="border border-dashed border-zinc-300 bg-white px-8 py-12 text-center">
      <p className="text-sm font-medium text-zinc-700">本课程还没有卡片</p>
      <p className="mt-1 text-xs text-zinc-400">完成章节精读或融合精读后，卡片会显示在这里。</p>
      <Link to="/workbench/chapter" className="mt-4 inline-flex bg-zinc-900 px-4 py-2 text-sm font-medium text-white">
        返回章节精读
      </Link>
    </div>
  );
