...
export default function SourceDetail() {
  ...
  useEffect(() => {
    if (selectedCourseId)
      loadSources(selectedCourseId).catch((err: unknown) =>
    ...
    <div className="max-w-2xl space-y-6 animate-in">
      ...
      {existingSources.length > 0 && (
        <section aria-labelledby="existing-sources-title" className="border-t border-zinc-200 pt-4">
          <h2 id="existing-sources-title" className="mb-2 text-xs font-medium text-zinc-500">
            已有教材 · {existingSources.length}
          </h2>
          <div className="divide-y divide-zinc-100">
            {existingSources.map((source) => (
              <div key={source.id} className="flex min-h-12 items-center justify-between gap-3 py-2">
                <p className="min-w-0 truncate text-sm font-medium text-zinc-900" title={source.title}>
                  {source.title}
                </p>
                <span className="shrink-0 text-xs text-zinc-400">{source.status}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {existingSources
        .filter((source) => source.file_path.toLowerCase().endsWith(".pdf"))
        .map((source) => (
          <OcrWorkflowPanel key={`ocr-${source.id}`} source={source} />
        ))}
    </div>
  );
}
