...
import { chapterOptionLabel, type SourceChapterGroup } from "./sourceChapterGroups";
    ...
    <div className="space-y-3">
      {groups.map(({ source, chapters, completedCount }) => {
        ...
        return (
          <section
            key={source.id}
            data-testid={`source-group-${source.id}`}
                  ...
                  const content = (
                    <>
                      ...
                      {chapter.status === "COMPLETED" && (
                        <CircleCheck size={14} className="ml-auto shrink-0 text-emerald-600" />
                      )}
                    </>
                  );
                  return chapterHref ? (
