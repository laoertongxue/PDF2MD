...
import { requireValue } from "../../test/requireValue";
...
const topic = (overrides: Partial<CourseTopic> = {}): CourseTopic => ({
  ...
  seq: 0,
  title: "战略选择",
  description: "比较不同战略路径",
  generation_reason: "跨教材共同解释决策",
  status: "DRAFT",
  confirmed: false,
  stale_reason: "",
  chapter_ids: ["ch1"],
...
function reset(overrides: Record<string, unknown> = {}) {
  state = {
    selectedCourseId: "c1",
    courses: [{ id: "c1", title: "MBA", description: "", root_dir: "/mba" }],
    sources: { c1: sources },
    chapters,
    topicsByCourse: { c1: [topic(), topic({ id: "t2", seq: 1, title: "组织协同", chapter_ids: ["ch3"] })] },
    topicActions: {},
    ...actions,
    ...overrides,
  };
...
describe("TopicMap", () => {
  ...
  it("renders topic status, rationale, uncovered chapters and grouped mapping checkboxes with counts", async () => {
    reset({
      chapters: {
        ...chapters,
        s1: requireValue(chapters.s1, "source s1 chapters").map((chapter) => ({ ...chapter, status: "COMPLETED" })),
      },
    });
    renderMap();
  ...
  it("renames, creates, reorders and deletes through store actions", async () => {
    ...
    await userEvent.type(name, "战略判断");
  ...
  it("merges at least two topics and splits only with a name and chapter", async () => {
    reset({
      topicsByCourse: {
        c1: [
          topic({ chapter_ids: ["ch1", "ch2"] }),
          topic({ id: "t2", seq: 1, title: "组织协同", chapter_ids: ["ch3"] }),
        ],
      },
    });
    renderMap();
    ...
    expect(actions.mergeTopics).toHaveBeenCalledWith(
      "c1",
      expect.objectContaining({ topic_ids: ["t1", "t2"], chapter_ids: ["ch1", "ch2", "ch3"] }),
    );
    ...
    expect(actions.splitTopic).toHaveBeenCalledWith(
      "t1",
      expect.objectContaining({ title: "细分战略", new_chapter_ids: ["ch1"] }),
    );
  });
  ...
  it("opens the stale old result route and renders ordered blocks instead of the editor", async () => {
    ...
    reset({
      topicsByCourse: { c1: [topic({ status: "STALE", stale_reason: "映射已变更" })] },
      topicBlocksById: {},
      loadTopicBlocks: vi.fn(async () => {
        state = { ...state, topicBlocksById: { t1: blocks } };
        return blocks;
      }),
    });
    const view = render(
      <MemoryRouter initialEntries={["/workbench/courses/c1/topics"]}>
        <Routes>
          <Route path="/workbench/courses/:courseId/topics" element={<TopicMap />} />
          <Route
            path="/workbench/courses/:courseId/topics/:topicId"
            element={<TopicMap initialTopicId="t1" oldResult />}
          />
        </Routes>
      </MemoryRouter>,
    );
    ...
    view.rerender(
      <MemoryRouter initialEntries={["/workbench/courses/c1/topics/t1"]}>
        <Routes>
          <Route
            path="/workbench/courses/:courseId/topics/:topicId"
            element={<TopicMap initialTopicId="t1" oldResult />}
  ...
  it("shows explicit empty and Chinese error states for old results", async () => {
    ...
    const empty = render(
      <MemoryRouter>
        <TopicMap initialTopicId="t1" oldResult />
      </MemoryRouter>,
    );
    ...
    reset({
      topicBlocksById: {},
      loadTopicBlocks: vi.fn().mockRejectedValueOnce(new Error("旧结果加载失败，请稍后重试")),
    });
    render(
      <MemoryRouter>
        <TopicMap initialTopicId="t1" oldResult />
  ...
  it("ignores an old topic result failure after course and topic switch", async () => {
    ...
    reset({
      selectedCourseId: "c2",
      courses: [{ id: "c2", title: "新课程", description: "", root_dir: "/new" }],
      topicBlocksById: {},
      loadTopicBlocks,
    });
    view.rerender(
      <MemoryRouter>
        <TopicMap initialTopicId="t2" oldResult />
      </MemoryRouter>,
    );
  ...
  it("ignores an old course retry response after switching courses", async () => {
    ...
    actions.retryTopicSync.mockReturnValueOnce(
      new Promise<CourseTopic>((done) => {
        resolve = done;
      }),
    );
    reset({ topicsByCourse: { c1: [topic({ sync_status: "FAILED", sync_error: "Markdown 同步失败" })] } });
    ...
    reset({
      selectedCourseId: "c2",
      courses: [{ id: "c2", title: "新课程", description: "", root_dir: "/new" }],
      sources: { c2: sources.map((source) => ({ ...source, course_id: "c2" })) },
      topicsByCourse: {
        c2: [
          topic({ id: "new", course_id: "c2", title: "新主题", sync_status: "FAILED", sync_error: "新课程同步失败" }),
        ],
      },
    });
    view.rerender(
  ...
  it("ignores an old course load and disables repeated generation", async () => {
    let resolve!: () => void;
    actions.loadTopics.mockReturnValueOnce(
  ...
  it("does not let an old write failure change the new course busy or error state", async () => {
    ...
    actions.createTopic.mockReturnValueOnce(
      new Promise((_, fail) => {
        reject = fail;
      }),
    );
    ...
    reset({
      ...
      courses: [{ id: "c2", title: "新课程", description: "", root_dir: "/new" }],
      sources: { c2: sources.map((source) => ({ ...source, course_id: "c2" })) },
      topicsByCourse: { c2: [topic({ id: "new", course_id: "c2", title: "新主题" })] },
    });
    view.rerender(
      <MemoryRouter>
        <TopicMap />
