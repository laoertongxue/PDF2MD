...
describe("CardPool", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    actions.loadCourses.mockResolvedValue(undefined);
    actions.loadCourseCards.mockResolvedValue(cards);
  ...
  it("focuses, scrolls and highlights a card from the real cardId route", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/workbench/cards?cardId=c2"]}>
        <Routes>
    ...
    expect(screen.getByRole("link", { name: "增长主题" })).toHaveAttribute(
      "href",
      "/workbench/courses/course/fusion/t1",
    );
  });
  ...
  it("clears filters that hide a route target and announces the adjustment", async () => {
    function RouteTarget() {
      ...
      return (
        <>
          <button onClick={() => navigate("/workbench/cards?cardId=c2")}>定位增长卡片</button>
          <CardPool />
        </>
      );
    }
    render(
      <MemoryRouter initialEntries={["/workbench/cards"]}>
        <Routes>
          <Route path="/workbench/cards" element={<RouteTarget />} />
        </Routes>
      </MemoryRouter>,
    );
    await userEvent.click(screen.getByRole("button", { name: "仅看收藏" }));
  ...
  it("announces a missing route target without moving focus", async () => {
    render(
      <MemoryRouter initialEntries={["/workbench/cards?cardId=missing"]}>
        <Routes>
          <Route path="/workbench/cards" element={<CardPool />} />
        </Routes>
      </MemoryRouter>,
