...
const actions = {
  loadTopics: vi.fn(),
  loadTopicBlocks: vi.fn(),
  loadTopicCards: vi.fn(),
  loadTopicRuns: vi.fn(),
  loadSources: vi.fn(),
  loadChapters: vi.fn(),
...
const topic = (status = "COMPLETED") => ({
  ...
  status,
  confirmed: true,
  stale_reason: status === "STALE" ? "章节 ch1 已更新" : "",
  chapter_ids: ["ch1"],
  blocking_chapter_ids: status === "NOT_READY" ? ["ch1"] : [],
  sync_status: "SYNCED",
  sync_error: "",
});
...
function reset(status = "COMPLETED") {
  state = {
    ...
    sources: { c1: [{ id: "s1", course_id: "c1", kind: "main", file_path: "/a", title: "战略教材", status: "READY" }] },
    chapters: { s1: [{ id: "ch1", source_id: "s1", course_id: "c1", seq: 0, title: "竞争战略", status: "COMPLETED" }] },
    topicsByCourse: { c1: [topic(status)] },
    topicBlocksById: {
      t1: kinds.map((kind, i) => ({
        id: `b${i}`,
        topic_id: "t1",
        kind,
        content: kind.includes("mermaid") ? "flowchart LR\nA-->B" : `正文 ${kind} [《战略教材》·第 1 章]`,
        updated_at: i + 1,
      })),
    },
    topicCardsById: {
      ...
      t1: [
        {
          ...
          input_fingerprint: "x",
          output: "",
          error: status === "FAILED" ? "topic round execution failed" : "",
          started_at: 1,
          finished_at: 2,
        },
      ],
...
describe("TopicFusion", () => {
  ...
  it("renders the fixed 15 sections, source routes, cards and editable Mermaid", async () => {
    render(
      <MemoryRouter>
        <TopicFusion courseId="c1" topicId="t1" />
      </MemoryRouter>,
    );
    ...
    expect(
      requireAt(screen.getAllByRole("link", { name: "[《战略教材》·第 1 章]" }), 0, "chapter source link"),
    ).toHaveAttribute("href", "/workbench/chapter?chapterId=ch1");
    expect(screen.getByText("卡片 8")).toBeInTheDocument();
    const editor = requireAt(screen.getAllByRole("textbox", { name: /Mermaid 源码/ }), 0, "Mermaid editor");
  ...
  it("disables NOT_READY, allows STALE, and keeps old content with a safe FAILED error", () => {
    ...
    expect(screen.getByText(/竞争战略/)).toBeInTheDocument();
    first.unmount();
    reset("STALE");
    const second = render(
      <MemoryRouter>
  ...
  it("ignores an old recovery failure after switching course and topic", async () => {
    ...
    actions.recoverTopic.mockReturnValueOnce(
    ...
    state = {
      ...
      topicsByCourse: { c2: [{ ...topic("RUNNING"), id: "t2", course_id: "c2", title: "新主题" }] },
      topicBlocksById: { t2: [] },
      topicCardsById: { t2: [] },
      topicRunsById: { t2: [] },
      sources: { c2: [] },
    ...
    view.rerender(
      <MemoryRouter>
        <TopicFusion courseId="c2" topicId="t2" />
      </MemoryRouter>,
    );
  ...
  it("ignores an old run failure after switching page", async () => {
    let rejectOld!: (reason: Error) => void;
    actions.runTopicHybrid.mockReturnValueOnce(
      new Promise((_, reject) => {
        rejectOld = reject;
      }),
    );
    ...
    state = {
      ...state,
      topicsByCourse: { c2: [{ ...topic("READY"), id: "t2", course_id: "c2", title: "新主题" }] },
      topicBlocksById: { t2: [] },
      topicCardsById: { t2: [] },
      topicRunsById: { t2: [] },
      sources: { c2: [] },
  ...
  it("keeps Mermaid save loading independent and ignores its late success after switching page", async () => {
    let resolveOld!: () => void;
    actions.saveTopicBlock.mockReturnValueOnce(
    ...
    await userEvent.clear(editor);
    await userEvent.type(editor, "flowchart LR\nX-->Y");
    const save = requireAt(screen.getAllByRole("button", { name: "保存 Mermaid" }), 0, "save button");
    await userEvent.click(save);
    expect(save).toBeDisabled();
    state = {
      ...state,
      topicsByCourse: { c2: [{ ...topic("COMPLETED"), id: "t2", course_id: "c2", title: "新主题" }] },
      topicBlocksById: { t2: [] },
      topicCardsById: { t2: [] },
      topicRunsById: { t2: [] },
      sources: { c2: [] },
