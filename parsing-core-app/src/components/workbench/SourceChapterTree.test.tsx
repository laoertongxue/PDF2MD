...
import type { SourceChapterGroup } from "./sourceChapterGroups";
import SourceChapterTree from "./SourceChapterTree";
...
describe("SourceChapterTree", () => {
  ...
  it("keeps same-name chapters under their source groups and selects by unique chapter id", async () => {
    ...
    await userEvent.click(requireAt(sameChapters, 1, "second source chapter"));
  ...
  it("collapses one source without hiding chapters in another source", async () => {
    ...
    expect(within(firstGroup).getByRole("button", { name: "展开《战略管理》" })).toHaveAttribute(
      "aria-expanded",
      "false",
    );
  });
