...
import type { TopicNoteBlock } from "../../api/workbenchTypes";
...
const SECTIONS = [
  ...
  ["plain_explanation", "通俗、有趣、生活化的解释"],
  ["textbook_cases", "教材案例解读"],
  ["real_world_problem_solving", "现实案例与问题解决"],
  ["integrated_framework", "综合分析框架"],
  ["application_methods", "实际应用方法"],
  ["further_thinking", "延伸思考"],
  ["knowledge_mermaid", "Mermaid 知识结构图"],
  ["application_mermaid", "Mermaid 应用流程图"],
  ["cards", "写作卡片"],
] as const;
...
export default function TopicFusion({ courseId, topicId }: { courseId: string; topicId: string }) {
  ...
  const loadTopicCards = store.loadTopicCards;
  const loadTopicRuns = store.loadTopicRuns;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      loadSources(courseId).then((sources) => Promise.all(sources.map((source) => loadChapters(source.id)))),
  ...
  const sourceTargets = useMemo(() => {
    ...
    for (const source of sources) {
      for (const chapter of store.chapters[source.id] ?? [])
        result.set(`[《${displayTitles.get(source.id)}》·第 ${chapter.seq + 1} 章]`, chapter.id);
    }
    return result;
  ...
  const latestSuccess = [...runs]
    .filter((run) => run.status === "COMPLETED" && run.finished_at)
    .sort((a, b) => (b.finished_at ?? 0) - (a.finished_at ?? 0))[0];
  const latestFailure = [...runs]
  ...
  const recover = async () => {
    ...
    setError(null);
    try {
      await store.recoverTopic(topicId);
    } catch (reason) {
  ...
  if (!topic && error)
    return (
      <div role="alert" className="border-l-2 border-red-500 bg-red-50 px-4 py-4 text-sm text-red-800">
        <p>{error}</p>
        <button
          type="button"
          onClick={() => setLoadAttempt((attempt) => attempt + 1)}
          className="mt-3 border border-red-200 bg-white px-3 py-2 text-sm font-medium"
        >
          重试加载
        </button>
      </div>
    ...
    <div className="grid min-w-0 border-y border-zinc-200 md:grid-cols-[220px_minmax(0,1fr)]">
      <aside className="border-b border-zinc-200 bg-zinc-50 md:border-b-0 md:border-r" aria-label="课程主题列表">
        ...
        {topics.map((item) => (
          <Link
            key={item.id}
            to={`/workbench/courses/${courseId}/fusion/${item.id}`}
            className={`block border-b border-zinc-100 px-4 py-3 ${item.id === topicId ? "bg-emerald-50" : "hover:bg-white"}`}
          >
            <span className="block break-words text-sm font-medium">{item.title}</span>
            <span className="mt-1 block text-xs text-zinc-500">{STATUS[item.status] ?? item.status}</span>
          </Link>
        ))}
      </aside>
      <main className="min-w-0 px-4 py-5 sm:px-6 lg:px-10">
        <header className="border-b border-zinc-200 pb-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              <h1 className="break-words text-2xl font-semibold">{topic.title}</h1>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-500">
                <span>{STATUS[topic.status] ?? topic.status}</span>
                <span>
                  最后成功：
                  {latestSuccess?.finished_at ? new Date(latestSuccess.finished_at * 1000).toLocaleString() : "暂无"}
                </span>
              </div>
            </div>
            <div className="flex shrink-0 flex-wrap gap-2 whitespace-nowrap">
              ...
              {["RUNNING", "FAILED"].includes(topic.status) && (
                <button
                  ...
                  aria-label="检查并恢复"
                  title="检查运行状态并恢复过期任务"
                  className="inline-flex items-center gap-2 border border-zinc-200 px-3 py-2 text-sm disabled:opacity-40"
                >
                  {recovering ? <Loader2 size={16} className="animate-spin" /> : <RotateCcw size={16} />}检查并恢复
                </button>
              )}
            </div>
          </div>
          {topic.stale_reason && <p className="mt-3 text-sm text-amber-700">过期原因：{topic.stale_reason}</p>}
          {topic.blocking_chapter_ids.length > 0 && (
            <div className="mt-3 border-l-2 border-amber-400 bg-amber-50 px-3 py-2 text-sm text-amber-900">
              阻塞章节：{topic.blocking_chapter_ids.map((id) => chapterById.get(id)?.title ?? id).join("、")}
            </div>
          )}
          {topic.sync_status === "FAILED" && (
            <div className="mt-3 flex items-center gap-3 text-sm text-red-700">
              <span>Markdown 同步失败，已保留数据库版本</span>
              <button
                type="button"
                onClick={() => store.retryTopicSync(topicId)}
          ...
          {(error || (topic.status === "FAILED" && latestFailure?.error)) && (
            <p role="alert" className="mt-3 border-l-2 border-red-500 bg-red-50 px-3 py-2 text-sm text-red-800">
        ...
        <div className="mx-auto max-w-3xl divide-y divide-zinc-200">
          {SECTIONS.map(([kind, title], index) => (
            <section key={kind} className="py-7">
              <h2 className="text-lg font-semibold">
                {index + 1}. {title}
              </h2>
              <div className="mt-4">
                {kind === "cards" ? (
                  <CardSection cards={cards} sourceTargets={sourceTargets} />
                ) : (
                  <BlockSection
                    block={blocks.find((item) => item.kind === kind)}
                    sourceTargets={sourceTargets}
                    onSave={(content, expected) => saveBlock(kind, content, expected)}
                  />
                )}
              </div>
            </section>
  ...
  for (const source of sources) {
    ...
    if (assigned.has(normalized(display))) {
      for (let suffix = 2; suffix <= 10_000; suffix += 1) {
        ...
        if (!reserved.has(key) && !assigned.has(key)) {
          display = candidate;
          break;
        }
      }
    }
    assigned.add(normalized(display));
...
function RichText({ text, sourceTargets }: { text: string; sourceTargets: Map<string, string> }) {
  return (
    <div className="space-y-3 text-sm leading-7 text-zinc-700">
      {text.split(SOURCE_RE).map((part, index) => {
        const chapterId = sourceTargets.get(part);
        if (chapterId)
          return (
            <Link
              key={`${part}-${index}`}
              to={`/workbench/chapter?chapterId=${chapterId}`}
        ...
        if (/^\[《[^\]\n]+》·第\s*\d+\s*章\]$/.test(part))
          return (
            <span key={`${part}-${index}`} className="text-zinc-500">
              {part}
            </span>
          );
...
function BlockSection({
  ...
  block: TopicNoteBlock | undefined;
  sourceTargets: Map<string, string>;
  onSave: (content: string, expectedContent: string) => Promise<boolean>;
}) {
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="block text-xs font-medium text-zinc-500">
        Mermaid 源码
        <textarea
          aria-label={`${block.kind} Mermaid 源码`}
          value={code}
          onChange={(event) => {
          ...
          spellCheck={false}
          className="mt-2 h-40 w-full resize-y border border-zinc-200 p-3 font-mono text-xs leading-5"
        />
      </label>
      <button
        type="button"
        onClick={async () => {
          ...
          setSaving(true);
          const ok = await onSave(code, block.content);
          if (saveGeneration.current === generation) {
            setSaved(ok);
            setSaving(false);
          }
        }}
        disabled={saving || !code.trim() || code === block.content}
        className="inline-flex items-center gap-2 border border-zinc-200 px-3 py-2 text-sm disabled:opacity-40"
      >
        {saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}保存 Mermaid
      </button>
      {saved && <span className="ml-3 text-xs text-emerald-700">已保存并同步 Markdown</span>}
    </div>
  );
}
...
function CardSection({
  ...
  cards: Array<{ id: string; card_type: string; title: string; content: string; source_refs: string[] }>;
  sourceTargets: Map<string, string>;
}) {
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {cards.map((card) => (
        <article key={card.id} className="border-l-2 border-emerald-500 pl-4">
          <p className="text-xs text-zinc-500">{card.card_type}</p>
          <h3 className="mt-1 text-sm font-semibold">{card.title}</h3>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-700">{card.content}</p>
          <div className="mt-3 flex flex-wrap gap-2">
            {card.source_refs.map((ref) =>
              sourceTargets.has(ref) ? (
                <Link
                  key={ref}
                  to={`/workbench/chapter?chapterId=${sourceTargets.get(ref)}`}
                  className="text-xs text-emerald-700 underline"
                >
                  {ref}
                </Link>
              ) : (
                <span key={ref} className="text-xs text-zinc-500">
                  {ref}
                </span>
              ),
            )}
