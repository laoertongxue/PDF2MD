    ...
    <div className="grid min-w-0 border-y border-zinc-200 md:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <main className="min-w-0 px-4 py-5 sm:px-6 lg:px-10">
        <header className="border-b border-zinc-200 pb-5">
          ...
          {(error || (topic.status === "FAILED" && latestFailure?.error)) && <p role="alert" className="mt-3 border-l-2 border-red-500 bg-red-50 px-3 py-2 text-sm text-red-800">{error ?? latestFailure?.error}</p>}
        </header>
