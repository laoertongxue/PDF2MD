...
export default function BatchSubmit() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      alert("提交失败: " + (error instanceof Error ? error.message : "未知"));
      setSubmitting(false);
    }
  };
      ...
      {pasting && (
        <div className="rounded-lg border border-zinc-200 bg-white p-5 animate-in">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setPasting(false)} className="text-zinc-400 hover:text-zinc-600">
              <X size={15} />
            </button>
          </div>
          <textarea
      ...
      {paths.length > 0 && (
        <div className="rounded-lg border border-zinc-200 bg-white animate-in">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100">
            ...
            <button onClick={() => setPaths([])} className="text-xs text-zinc-400 hover:text-red-500">
              清空
            </button>
          </div>
          <div className="max-h-56 overflow-y-auto divide-y divide-zinc-50">
            {paths.map((p, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-2.5 text-sm hover:bg-zinc-50">
                ...
                <button onClick={() => remove(i)} className="text-zinc-300 hover:text-red-500 shrink-0">
                  <X size={14} />
                </button>
              </div>
            ))}
