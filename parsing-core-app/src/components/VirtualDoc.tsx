  ...
  useEffect(() => {
    ...
    const cb = () => {
      for (let i = secs.length - 1; i >= 0; i--) {
        ...
        if (s && s.offsetTop <= el.scrollTop + 100) {
          setActive(i);
          break;
        }
      }
      ...
      <div ref={ref} className="flex-1 overflow-y-auto space-y-16 pb-24">
        {secs.map((sec, i) => (
          <section key={i} id={`s-${i}`} className="scroll-mt-20">
            ...
            {/* Original */}
            <div
              className="prose prose-sm prose-zinc max-w-none mb-8
            ...
            {sec.ai && (
              <div className="relative rounded-lg border border-blue-100 bg-blue-50/30 p-5 mb-8">
                <span className="absolute -top-2.5 left-4 text-[11px] font-semibold bg-blue-600 text-white px-2.5 py-0.5 rounded-full">
                  AI 解读
                </span>
                <div
                  className="prose prose-sm prose-zinc max-w-none mt-2
